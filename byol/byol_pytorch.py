...
class BYOL(nn.Module):
    def __init__(
        ...
        self.forward(torch.randn(2, image_size, device=device))
    ...
    def forward(
        ...
        # 将 x 转换为 NumPy 数组
        x_np = x.cpu().numpy()  # 假设 x 是一个张量，使用 .cpu() 以确保在 CPU 上进行转换

        # 遍历每个样本
        for v in x_np:
            image_one.append(self.augment1(v))  # 假设 augment1 和 augment2 接受 numpy 数组
            image_two.append(self.augment2(v))

        # 将增强后的结果转换回 PyTorch 张量
        image_one = torch.tensor(image_one)
        image_two = torch.tensor(image_two)

        image_one.resize_(image_one.size()[0], 1, image_one.size()[1])
        image_two.resize_(image_two.size()[0], 1, image_two.size()[1])
        image_one, image_two = image_one.float(), image_two.float()

        images = torch.cat((image_one, image_two), dim = 0).cuda()
